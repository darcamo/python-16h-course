...
function update_baterias() {
    ...
    if (currentSlideId === "baterias") {
        ...
        svg.selectAll(".python").classed("hidden", true);
        ...
        case 0:
            svg.selectAll(".python").classed("hidden", false);
            svg.selectAll(".numpy").classed("hidden", true);
            svg.selectAll(".scipy").classed("hidden", true);
            svg.selectAll(".plots").classed("hidden", true);
            svg.selectAll(".ipython").classed("hidden", true);
...
function create_1d2d_array(id, data) {
    ...
    var svg = d3.select(id)
        .html("") // Remove previous content such that if the function is called
                  // twice we don't have multiple svgs
        .append("svg")
...
var already_added_numpy_1d2d_array = false;
function update_numpy_1d2d_array()
{
    if(already_added_numpy_1d2d_array === false) {
        var currentSlideId = Reveal.getCurrentSlide().id;
        if (currentSlideId === "numpy_arrays") {
            var currentFragment = Reveal.getIndices().f;
        ...
        already_added_numpy_1d2d_array = true;
    }
}
